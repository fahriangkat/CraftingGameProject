...
public class SlideInventory : MonoBehaviour
{
    ...
    {
        // Animate the inventory panel's position from its current position to the target position
        LeanTween.move(inventoryPanel, targetPosition, slideDuration)
            .setEaseInOutQuad()
            .setOnComplete(OnSlideComplete); // Callback for when the sliding animation completes
